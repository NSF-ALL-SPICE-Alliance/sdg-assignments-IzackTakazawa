...
goal_3_vs_goal_4_scatter <- ggplot(sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal() +
  scale_fill_viridis_d() +
  labs(title = "The Relationship Between Goal 3 vs Goal 4 Scores",
       x = "SDG 3 Score",
       y = "SDG 4 Score",
       color = "Region")

ggplotly(goal_3_vs_goal_4_scatter)
```
...
leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("viridis", goal_3_score)(goal_3_score), label = mytext)
```
