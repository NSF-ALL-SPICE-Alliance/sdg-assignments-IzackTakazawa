...
```{r}
ggplot(data = sdr_data, aes(x= goal_4_score, y = goal_5_score, color = goal_6_score)) +
  geom_point()
...
In the scatterplot I created there seems to be a similar trend where the darkest blue points look to have mid-range SDG 4 and SDG 5 scores. Where as the lightest blue points tend to have high SDG 4 / SDG 5 scores. Compared to the last scatterplot, this plot is a lot less condescend in one area and more spread out, but there still is a trend.
...
From looking at the histogram, countries in the Sub-Saharan African region have a lower Goal 3 score compared to the OECD region that have countries that fall in the higher end of the Goal 3 spectrum. Another interesting trend I noticed from this vizualization is that the MENA, LAC, East & South Asia, and E. Europe & C. Asia, have countries that have a very in between Goal 3 score and these regions all fall in between the Sub-Saharan African and OECD regions on the Goal 3 spectrum.
